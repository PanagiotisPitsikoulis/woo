...
<li <?php wc_product_class('card bg-base-100 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1', $product); ?>>
	...
	<figure class="relative px-4 pt-4">
		<?php if ($product->is_on_sale()) : ?>
			<div class="absolute top-6 right-6 z-10">
				<span class="badge badge-secondary"><?php esc_html_e('Sale!', 'tw'); ?></span>
			</div>
		<?php endif; ?>
	...
	<div class="card-body">
		...
		<h2 class="card-title hover:text-primary transition-colors duration-200">
			<?php
			/**
		...
		<div class="card-actions justify-end mt-4">
			...
			<a href="<?php echo esc_url($product->add_to_cart_url()); ?>" class="btn btn-primary btn-sm">
				...
				if ($product->is_type('variable')) {
					esc_html_e('Select Options', 'tw');
				} elseif ($product->is_type('grouped')) {
					esc_html_e('View Products', 'tw');
				} else {
					esc_html_e('Add to Cart', 'tw');
				}
				?>
