...
function tw_enqueue_scripts() {
	...
	// DaisyUI and WooCommerce fixes
	wp_enqueue_style(
		'tw-daisy-woo-fixes',
		get_template_directory_uri() . '/css/daisy-woo-fixes.css',
		array('tw-style'),
		TW_VERSION
	);
	...
	wp_localize_script(
		...
		array(
			...
			'nonce'   => wp_create_nonce('tw_theme_nonce'),
		)
	);
 ...
 */
require get_template_directory() . '/inc/class-wp-tailwind-navwalker.php';
 ...
 */
function tw_load_theme_textdomain() {
	load_theme_textdomain('tw', get_template_directory() . '/languages');
}
add_action('after_setup_theme', 'tw_load_theme_textdomain');

/**
 * AJAX function to save theme preference
 */
function tw_save_theme_preference() {
	// Check nonce for security
	check_ajax_referer('tw_theme_nonce', 'nonce');
	...
	if (!empty($theme)) {
		// Save to theme mod
		set_theme_mod('tw_theme_style', $theme);
...
add_action('wp_ajax_tw_save_theme', 'tw_save_theme_preference');
add_action('wp_ajax_nopriv_tw_save_theme', 'tw_save_theme_preference');

/**
 * Enqueue admin scripts
 */
function tw_enqueue_admin_scripts() {
	wp_enqueue_script(
		'tw-admin-script',
...
add_action('admin_enqueue_scripts', 'tw_enqueue_admin_scripts');

/**
 * Add proper body classes for admin bar
 */
function tw_admin_bar_body_class($classes) {
	if (is_admin_bar_showing()) {
		$classes[] = 'admin-bar';
	}
	return $classes;
}
add_filter('body_class', 'tw_admin_bar_body_class');

/**
 * WooCommerce setup function.
 */
function tw_woocommerce_setup() {
	add_theme_support('woocommerce');
	add_theme_support('wc-product-gallery-zoom');
	add_theme_support('wc-product-gallery-lightbox');
	add_theme_support('wc-product-gallery-slider');
}
add_action('after_setup_theme', 'tw_woocommerce_setup');
