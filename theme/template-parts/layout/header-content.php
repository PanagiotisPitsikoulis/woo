	...
	<div class="navbar container mx-auto px-4">
		<div class="navbar-start">
			...
			<div class="flex items-center">
				...
				if ($custom_logo_id) {
					...
					if ($logo) {
						echo '<a href="' . esc_url(home_url('/')) . '" class="mr-3">';
				...
				if (is_front_page()) :
					?>
					<h1 class="text-xl font-bold"><a href="<?php echo esc_url(home_url('/')); ?>" class="no-underline text-inherit" rel="home"><?php bloginfo('name'); ?></a></h1>
					<?php
				else :
					?>
					<p class="text-xl font-bold"><a href="<?php echo esc_url(home_url('/')); ?>" class="no-underline text-inherit" rel="home"><?php bloginfo('name'); ?></a></p>
					<?php
				endif;
				?>
			</div>
		...
		<div class="navbar-end">
			<?php if (class_exists('WooCommerce')) : ?>
				<div class="dropdown dropdown-end mr-2">
					...
					<div tabindex="0" class="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
						<div class="card-body">
							<span class="font-bold text-lg"><?php echo sprintf(_n('%d item', '%d items', WC()->cart->get_cart_contents_count(), 'tw'), WC()->cart->get_cart_contents_count()); ?></span>
							<span class="text-info"><?php echo WC()->cart->get_cart_total(); ?></span>
							<div class="card-actions">
								<a href="<?php echo esc_url(wc_get_cart_url()); ?>" class="btn btn-block"><?php esc_html_e('View cart', 'tw'); ?></a>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="dropdown dropdown-end ml-2">
				...
				<ul tabindex="0" class="menu dropdown-content z-[1] shadow bg-base-100 rounded-box max-h-96 overflow-y-auto w-52 mt-3">
					<li class="menu-title"><?php esc_html_e('Select Theme', 'tw'); ?></li>
					<?php
					$themes = array(
						'light' => 'Light',
						'dark' => 'Dark',
					...
					foreach ($themes as $value => $label) :
						$is_active = ($value === $current_theme) ? ' active' : '';
					?>
						<li>
							<a href="#" class="theme-item<?php echo $is_active; ?>" data-theme="<?php echo esc_attr($value); ?>">
